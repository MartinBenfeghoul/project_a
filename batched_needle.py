"""
Needle-in-a-haystack evaluation for MLP-compressed KV caches.

Generates passkey retrieval sequences, runs them through a model to get KV caches,
...
def evaluate_sequence(
    ...
    with torch.no_grad():
        past_key_values, original_input_id = generate_kv_batched(
            ...
            1,
            tokenizer,
            device,  # batch_size=1 since processing one seq at a time
        )
        ...
        for target_perc in target_percentages:
            ...
            for layer_idx, layer in enumerate(past_key_values.layers):
                ...
                errors = F.mse_loss(v_approx, values, reduction="none").mean(
                    dim=-1
                )
...
def run_experiment(model, tokenizer, splits, config, device):
    ...
    num_ml_epochs = (
        OmegaConf.load(
            os.path.join(os.path.dirname(pretrained_mlps_path), "config.yaml")
        ).training.num_meta_epochs
    ...
    if pretrained_mlps_path:
        pretrained_checkpoint = torch.load(
            pretrained_mlps_path, map_location=device
        )
        ...
        if inner_lr_params is not None:
            ...
            print(
                f"Loaded meta-learned inner LR params from {pretrained_mlps_path}"
            )
    ...
    for split in splits:
        ...
        for _ in range(num_samples_per_split):
            ...
            avg_nll_, kv_cache = avg_nll([seq], model, 1, tokenizer, device)
            _, num_head, num_token, head_dim = kv_cache.layers[0].keys.shape

            layer_mlps = [
                MLP(num_heads=num_head, head_dim=head_dim).to(device)
            ...
            if pretrained_checkpoint is not None:
                for i, mlp in enumerate(layer_mlps):
                    ...
                    if layer_key not in pretrained_checkpoint:
                        raise KeyError(
                            f"Expected '{layer_key}' in pretrained checkpoint but it was not found."
                        )
                    mlp.load_state_dict(pretrained_checkpoint[layer_key])

            layer_mlps = train_mlps(
                layer_mlps,
                kv_cache,
                config.training,
                inner_lr_params=inner_lr_params,
            )
            ...
            for perc_idx in range(len(target_percentages)):
                ...
                total_num_changed_kv[perc_idx] += results["num_changed_kv"][
                    perc_idx
                ]
        ...
        for perc_idx, target_perc in enumerate(target_percentages):
            ...
            print(
                f"Avg KV changed: {avg_changed_kv:.2f}, Percentage: {percentage_changed_kv:.2f}%"
            )
            ...
            print(
                f"Saved results for split {split}, compression {target_perc}%"
            )
