...
class VectorizedIndependentHeadMLP(nn.Module):
    def __init__(self, num_heads, head_dim, hidden_factor=2):
        ...
        self.net = torch.nn.Sequential(
            ...
            torch.nn.Conv1d(
                total_in, total_hidden, kernel_size=1, groups=num_heads
            ),
            torch.nn.GELU(),
            # Middle layers: Keep hidden dim per head
            torch.nn.Conv1d(
                total_hidden, total_hidden, kernel_size=1, groups=num_heads
            ),
            torch.nn.GELU(),
            torch.nn.Conv1d(
                total_hidden, total_hidden, kernel_size=1, groups=num_heads
            ...
            torch.nn.Conv1d(
                total_hidden, total_in, kernel_size=1, groups=num_heads
            ),
        )
...
if __name__ == "__main__":
    ...
    if load_ds_from_disk:
        ...
        datasets = [
            load_from_disk(f"./datasets/passkey_seq_len_{split}")
            for split in splits
        ]
    else:
    ...
    for ds_idx, ds in enumerate(datasets):
        ...
        for seq_idx, seq in enumerate(datasets[ds_idx]):
            ...
            with torch.no_grad():
                ...
                for perc_idx, target_perc in enumerate(target_percentages):
                    ...
                    threshold_val = torch.topk(
                        flat_errors, k, largest=False
                    ).values[-1]
                    ...
                    for layer_idx, layer in enumerate(past_key_values.layers):
                        ...
                        layer_errors = F.mse_loss(
                            v_approx_3d, v_orig_3d, reduction="none"
                        ).mean(dim=-1)
                    ...
                    accuracies[perc_idx] += (
                        1 if seq["answer"] == clean(pred) else 0
                    )
        ...
        for perc_idx, total_nll in enumerate(new_total_nlls):
            ...
            new_avg_nll_change_perc = (
                total_nlls_perc_change[perc_idx] / num_samples
            )
            ...
            normalized_layers = {
                ...
                for layer_num, total_dv in layer_decompositions[
                    perc_idx
                ].items()
            }

            print(
                f"Overall acc for modified cache with threshold {splits[ds_idx]} is {accuracy:.4f}"
            )
            # print(f'Overall nll for modified cache for sequence {seq_idx} with threshold {thresholds[idx]} is {new_avg_nll_:.4f}')
            print(
                f"Overall nll for modified cache for sequence {splits[ds_idx]} with percentage {target_percentages[perc_idx]} is {new_avg_nll_change_perc:.4f} official {percentage_changed_kv}"
            )
