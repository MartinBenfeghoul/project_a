...
from utils.rope import get_rope_theta, inverse_rope
...
def _support_kvs(model, prompt, key_config):
    ...
    return prepare_kvs(
        cache,
        rope_theta=get_rope_theta(model.config),
        dtype=torch.float32,
...
def test_inner_loop_keys_match_the_eval_reconstruction(monkeypatch):
...
def test_without_a_key_config_the_support_keys_stay_exact():
...
def _unrope(keys, model):
    from utils.rope import compute_rope_cos_sin

    cos, sin = compute_rope_cos_sin(
        keys.shape[2],
        keys.shape[-1],
        get_rope_theta(model.config),
        keys.device,
        keys.dtype,
    )
