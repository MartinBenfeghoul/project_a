...
def extract_kv_linear_init(model, per_head=False) -> list[torch.Tensor]:
    ...
    if per_head:
        ...
        W_linear = (torch.linalg.pinv(W_k) @ W_v).to(W_k_raw.dtype) # [layers, num_kv_heads, head_dim, head_dim]
    else:
        W_linear = (torch.linalg.pinv(W_k) @ W_v).to(W_k_raw.dtype)
        W_linear = W_linear.view(len(layers), num_kv_heads, head_dim, num_kv_heads, head_dim)
