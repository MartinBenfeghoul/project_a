...
def main(
    ...
    with open(log_file, 'r') as f:
        ...
        for i, line in enumerate(lines):
            if 'Results saved to' in line:
                results_file = line.strip().split('Results saved to')[-1].strip()
    ...
    for i, (st, end, results_file) in enumerate(parts):
        ...
        if results_file and results_file.endswith('.json') and os.path.exists(
            results_file
        ):
            print(f"Results file {results_file} already exists, skipping extraction.")
        else:
            try:
