...
def make_predictions(batch_size, ds, model, tokenizer, device, past_key_values=None):
    ...
    for std in range(0, len(prompts), batch_size):
        ...
        inputs = tokenizer(prompts[std:end], return_tensors='pt', add_special_tokens=True, padding=True).to(device)
        with torch.no_grad():
            output = model(input_ids=inputs["input_ids"], past_key_values=past_key_values, use_cache=True) if past_key_values else model.generate(**inputs, max_new_tokens=16, do_sample=False, pad_token_id=tokenizer.pad_token_id)
...
def eval_model(ds, model, batch_size, tokenizer, device, past_key_values=None):
    preds = make_predictions(batch_size, ds, model, tokenizer, device, past_key_values)
    acc = 0
...
def update_cache(cache, updated_cache, num_head, token_idx, std=0, end=1):
    for layer, updated_layer in zip(cache.layers, updated_cache.layers):
        layer.keys[std:end, :, token_idx, :] = updated_layer.keys[std:end, :, token_idx, :]
...
def avg_nll(ds, model, batch_size, tokenizer, device, num_head=0, updated_cache=None, already_tokenized=False):
    ...
    for std in range(0, num_seq, batch_size):
        ...
        if updated_cache: cache = update_cache(cache, updated_cache, num_head, token_idx=0, std=std, end=end)
        ...
        for token_idx in range(1, seq_len - 1):
            ...
            if updated_cache: cache = update_cache(cache, updated_cache, num_head, token_idx, std=std, end=end)
